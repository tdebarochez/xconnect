...
const char *GetParsingFileName() { return g_parsingPath; }

int GetParsingLineNumber() { return g_parsingLineNb; }
...
int TokenTypeIs(token_type ty) { return (TOKENTYPE == ty); }
...
token_type ReadToken(FILE *fp) { // skip leading blanks
  char *buf;
  buf = TOKENBUFFER;
  fpos_t fpos;
  fgetpos(fp, &fpos);
  ...
  // It's an identifier.
  if ((car_read >= 'A' && car_read <= 'Z') ||
      (car_read >= 'a' && car_read <= 'z')) {
    *(buf++) = (char)car_read;
    while (1) {
      car_read = getc(fp);
      if ((car_read >= '0' && car_read <= '9') ||
          (car_read >= 'A' && car_read <= 'Z') ||
          (car_read >= 'a' && car_read <= 'z') || car_read == '-' ||
          car_read == '.' || car_read == '_') {
  ...
  // It's a number.
  if ((car_read >= '0' && car_read <= '9') || (car_read == '+') ||
      (car_read == '-')) {
    assert(buf - TOKENBUFFER < TOKENMAXSIZE * sizeof(char));
    *(buf++) = (char)car_read;
    while (1) {
...
const char *GetTokenBuffer() { return TOKENBUFFER; }
