      ...
      if (pView) {
        ...
        while ((*pdw) && (result == XC_RETURN_SUCCESS)) {
            ...
            if (pHdrW->size) {
              ...
              /*if (pHdrW->action == XC_ACTION_WRITETRUE) {
                      ...
                      if (!IsBadWritePtr((BYTE*)true_offset, pHdrW->size))
                              CopyMemory((BYTE*)true_offset, pNext,
              pHdrW->size);
              }
              else {
                      writedataptr = pNext;
          ...
          /*case XC_ACTION_READTOKEN:
                  {
                          ...
                          if (pHdrRT) {
                                  ...
                                  CopyMemory(pNext, (BYTE*)(&mvar.var_value.n),
          8);
                                  pNext += 8;
                                  pdw = (DWORD*)pNext;
                          }
          ...
          case XC_ACTION_SEARCH:
                  {
                          ...
                          if (pHdrS->size) {
                                  if (pHdrS->offset_from < pHdrS->offset_to) {
                                          for (os = pHdrS->offset_from; os <=
          pHdrS->offset_to; os++) {
                                                  if (!IsBadReadPtr((BYTE*)os,
          pHdrS->size)) {
                                                          eq = TRUE;
                                                          for (i = 0; i <
          pHdrS->size; i++) {
          ...
          FALSE;
                                                          ...
                                                          if (eq) {
                                                                  str =
          fopen("modules\\FDSConnection.out", "a+");
                                                                  sprintf(buf,
          "value found at %d\n", (long)os);
                                                                  fprintf(str,
          buf);
...
void module_init(HINSTANCE dllInstance) {
  ...
  if (RegisterClass(&wc)) {
    hWnd = CreateWindowEx(NULL, TEXT("FS98MAIN"), TEXT("FS98MAIN"), WS_POPUP, 0,
                          0, 10, 10, NULL, NULL, dllInstance, NULL);
    if (hWnd) {
