...
BOOL InitializeSocketServer() {
  ...
  if (!FDSConnectionWnd) {
          MessageBox(NULL, "Cannot find FDSConnection!",
  "FDSWideConnectionServer Error", MB_OK);
          return FALSE;
  }
  */
...
/*void SocketInstanceThread(LPVOID lpvParam)
{
        ...
        while ((ret > 0)  && (ret != SOCKET_ERROR)) {
                ...
                {
                        recv(sd2, data + sizeof(DWORD),
sizeof(FDS_ACTION_READ_HDR) - sizeof(DWORD), MSG_WAITALL);
                        pHdrR = (FDS_ACTION_READ_HDR*)pdw;
                        ret = recv(sd2, data + sizeof(FDS_ACTION_READ_HDR),
pHdrR->size, MSG_WAITALL);

                        if(ret>0 && ret!=SOCKET_ERROR)
                        {
                                size = sizeof(FDS_ACTION_READ_HDR) +
pHdrR->size;
                                ProcessSocketData(procPos,
sizeof(FDS_ACTION_READ_HDR) + pHdrR->size);
                        }
                ...
                else if(*pdw == FDS_ACTION_WRITE)
                {
                        ...
                        recv(sd2, data + sizeof(DWORD),
sizeof(FDS_ACTION_WRITE_HDR) - sizeof(DWORD), MSG_WAITALL);
                        pHdrW = (FDS_ACTION_WRITE_HDR*)pdw;
                        ret = recv(sd2, data + sizeof(FDS_ACTION_WRITE_HDR),
pHdrW->size, MSG_WAITALL);

                        if(ret>0 && ret!=SOCKET_ERROR)
                        {
                                size = sizeof(FDS_ACTION_WRITE_HDR) +
pHdrW->size;
                                ProcessSocketData(procPos,
sizeof(FDS_ACTION_WRITE_HDR) + pHdrW->size);
                        }
...
*/
    ...
    // MessageBox(NULL, "Error communication with FDSConnect",
    // "FDSWideConnectionServer Error", MB_OK);
  }
